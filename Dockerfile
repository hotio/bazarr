...
RUN apt update && \
    apt install -y --no-install-recommends --no-install-suggests \
        libxml2 libxslt1.1 \
        python-pip build-essential python-dev python-setuptools libxml2-dev libxslt1-dev && \
    pip --no-cache-dir install gevent==1.4.0 lxml==4.3.0 && \
# clean up
    apt purge -y python-pip build-essential python-dev python-setuptools libxml2-dev libxslt1-dev && \
    apt autoremove -y && \
    apt clean && \
